...
const dropdownOption = [
  { key: 1, label: '关闭', icon: () => <QnIcon icon="close"/> },
  { key: 2, label: '关闭其他', icon: () => <QnIcon icon="close-outline"/> },
  { key: 3, label: '关闭全部', icon: () => <QnIcon icon="close-filled"/> },
]
...
<template>
  ...
  <n-scrollbar x-scrollable ref="scrollbarRef" @wheel.prevent="wheel">
    <n-flex :wrap="false" :size="6" style="margin: 6px"><!-- 外部边距以及元素间的边距都是6 -->
      ...
      >
        <template #icon>
          <qn-icon v-if="value === i.key" icon="check-filled" :size="20"/>
          <qn-icon v-else icon="dot" depth="3" :size="20"/>
        </template>
        {{ i.label }}
      </n-tag>
    </n-flex>
